...
namespace delaunay {
    ...
    vector<CIDTriangle> Delaunay(vector<delaunay::Cv2>&vlist){
        ...
        //----call Delaunay
        vector<CIDTriangle> IDtriList=Delaunay(_vlist);//note: IDtriList is calculated on xz plane, so the IDtris' are CW on xz plane and CCW on xy plane.
        //----we want IDtris' CW on xy plane, so we should reverse them
        int nIDtri=(int)IDtriList.size();
        for(int i=0;i<nIDtri;i++){
            CIDTriangle&IDtri=IDtriList[i];
            //swap ID1 and ID2
            int temp=IDtri.vID[1];
            IDtri.vID[1]=IDtri.vID[2];
            IDtri.vID[2]=temp;
        }
        return IDtriList;
    }
